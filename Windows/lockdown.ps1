...
if ($option -eq 1) {
    ...
    Write-Warning "reking plebs and their shit passwords, changing to changeMe!123"
    Get-WmiObject win32_useraccount | Foreach-object {
    ([adsi]("WinNT://"+$_.caption).replace("\","/")).SetPassword("changeMe!123")
    }
    ...
    Write-Host "Setting Account Stuff with net accounts DONE" -ForegroundColor Green
	...
	$askuserservice = Read-Host "Bitch. Do you want to disable IIS Stuff? Y or N."

    if ($askuserservice -eq "Y") {
    ...
    $askuserserviceTFTP = Read-Host "Bitch. Do you want to disable TFTP? Y or N."
    if ($askuserserviceTFTP -eq "Y") {
	    dism /online /disable-feature /featurename:TFTP
    }

    $askuserserviceTelnet = Read-Host "Bitch. Do you want to disable telnet? Y or N." {
    ...
    Write-Warning "investigating shady processes with memory allocated over 20MBs"
    ...
    New-NetFirewallRule -DisplayName "METASPLOIT" -Direction Inbound -LocalPort 4444 -Protocol TCP -Action Block #Metasploit
    ...
    #Require Security Signature Workstation
    reg ADD HKLM\SYSTEM\CurrentControlSet\services\LanmanWorkstation\Parameters /v enablesecuritysignature /t REG_DWORD /d 1 /f
    
    #Enable Security Signature Workstation
